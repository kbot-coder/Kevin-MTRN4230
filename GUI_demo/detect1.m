...
%Soo Bin Han z3330656 
%last modified on 20th of April 2005
%Input image of table,output detection matrix
...
%     image2 = imread('IMG_043.jpg');
    ...
    elseif length(foundchoc)>0
        ...
        [points1, theIm, count, k ,chocname,g]=store_samplename(foundchoc);     
        if (chocname{count}(1:4)=='none')
        points2 = detectSURFFeatures(I2,'MetricThreshold',120); 
        else
    ...
    while n==1  
        ...
        if (chocname{count}(1:4)=='none')
        indexPairs = matchFeatures(features1{count}, features2,'MatchThreshold',100);
        else
