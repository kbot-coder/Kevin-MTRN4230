%ASSIGNMENT 1
%creator        : ian junardi yonowiharjo
%z3402188
%last edited 2:52 pm 16/04/2015


%chocolate detection.
function c = detect_chocolates_conv(im)
c=[];
im = imread(im);
...
target = im;
% target = imrotate(target,180);
targetgray = rgb2gray(target);
targetgray(750:900,1:1600) = 0;
target(750:900,1:1600,:) = 0;
targetgray(1:100,1:1600) = 0;
target(1:100,1:1600,:) = 0;
targetgray(:,1:600) = 0;
target(:,1:600,:) = 0;
...
%       figure(2);
      showMatchedFeatures(Samplegray{sampleIM}, targetgray, matchedCHOCPoints,matchedTARGETPoints, 'montage');
        ...
        if inX>=2 && status == 0 
            ...
            if (Check1 < (Check2 +10) && Check1 > (Check2-10)) && (Check1 < 230 &&(Check3> 60 && Check3<100))&& Check4 > 160
