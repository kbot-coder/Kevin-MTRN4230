...
%==========================================================================
function varargout = GUI_Design_2015_04_28(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Design_2015_04_28_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Design_2015_04_28_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%--------------------------------------------------------------------------
% Executes just before GUI_Design_2015_04_28 is made visible.
%--------------------------------------------------------------------------
function GUI_Design_2015_04_28_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.ShowCurrentJoint = ['0' , '0' , '0' , '0', '0', '0'];   % Joint 1->6 current status
...
set(handles.axes3, 'Xlim', [-0,1600], 'YLim', [0 1200]); % Setting limit axes tobe respect to the image resolution
...
handles.timer2= timer(...
    'ExecutionMode', 'fixedRate', ...               % Run timer repeatedly in fix rate
    'Period', 0.2, ...                              % Initial period is 0.1 sec.
    'TimerFcn', {@SendJOG,hObject,handles});
start(handles.timer2);

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GUI_Design_2015_04_28 wait for user response (see UIRESUME)
%uiwait(handles.figure1);
%--------------------------------------------------------------------------


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Design_2015_04_28_OutputFcn(hObject, eventdata, handles) 
...
function ShowCurrentJoint_Callback(hObject, eventdata, handles)
% --- Executes during object creation, after setting all properties.
function ShowCurrentJoint_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), ...
        get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 1 data 
    ,SPosition, handles.ShowCurrentJoint(2), handles.ShowCurrentJoint(3), ...   % & current data for other joints
    handles.ShowCurrentJoint(4), handles.ShowCurrentJoint(5), handles.ShowCurrentJoint(6));
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 2 data
    ,handles.ShowCurrentJoint(1), SPosition, handles.ShowCurrentJoint(3), ...   % & current data for other joints
    handles.ShowCurrentJoint(4), handles.ShowCurrentJoint(5), handles.ShowCurrentJoint(6));
sender(data);                                   % call sender function to send data angle joint goal
pause(.1);
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 3 data
    ,handles.ShowCurrentJoint(1), handles.ShowCurrentJoint(2), SPosition,...    % & current data for other joints
    handles.ShowCurrentJoint(4), handles.ShowCurrentJoint(5), handles.ShowCurrentJoint(6));
sender(data);                                   % call sender function to send data angle joint goal
pause(.1);
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 4 data
    ,handles.ShowCurrentJoint(1), handles.ShowCurrentJoint(2), handles.ShowCurrentJoint(3),...  % & current data for other joints
    SPosition, handles.ShowCurrentJoint(5), handles.ShowCurrentJoint(6));
sender(data);                                   % call sender function to send data angle joint goal
pause(.1);
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 5 data
    ,handles.ShowCurrentJoint(1), handles.ShowCurrentJoint(2),  handles.ShowCurrentJoint(3)...  % & current data for other joints
    , handles.ShowCurrentJoint(4), SPosition, handles.ShowCurrentJoint(6));
sender(data);                                   % call sender function to send data angle joint goal
pause(.1);
...
data=sprintf('[%s,%s,%s,%s,%s,%s]'...           % set data for sending ( new joint 6 data
    ,handles.ShowCurrentJoint(1), handles.ShowCurrentJoint(2),  handles.ShowCurrentJoint(3)...  % & current data for other joints
    , handles.ShowCurrentJoint(4), handles.ShowCurrentJoint(5), SPosition);
sender(data);                                   % call sender function to send data angle joint goal
pause(.1);
...
axes(handles.axes3);   
c = findChoc(imTable);   % HERE THE CHOC DETECTION
...
XR=1200-Y; YR=X-800; XR=XR*1.5; YR=YR*1.5;               % Convert & adjust the measurement from pixle to mm
...
set(handles.ShowCurrentJoint,'String',data);        % Show current joint 1-6 angle on the texbox

handles.ShowCurrentJoint(1) = sprintf('%d',handles.JointR(1));  % Store current Joint 1 angle on the string 
handles.ShowCurrentJoint(2) = sprintf('%d',handles.JointR(2));  % Store current Joint 2 angle on the string
handles.ShowCurrentJoint(3) = sprintf('%d',handles.JointR(3));  % Store current Joint 3 angle on the string
handles.ShowCurrentJoint(4) = sprintf('%d',handles.JointR(4));  % Store current Joint 4 angle on the string
handles.ShowCurrentJoint(5) = sprintf('%d',handles.JointR(5));  % Store current Joint 5 angle on the string
handles.ShowCurrentJoint(6) = sprintf('%d',handles.JointR(6));  % Store current Joint 6 angle on the string
...
function SendJOG(hObject,eventdata,hfigure,handles)
